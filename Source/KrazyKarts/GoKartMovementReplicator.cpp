...
{
	ServerState.LastMove = Move; //движение которое мы симулировали записываем как последнее движение указанное на сервере, которые последний раз симулировали
...
{
	ClientTimeBetweenLastUpdate += DeltaTime;
	
	if (ClientTimeBetweenLastUpdate < KINDA_SMALL_NUMBER) return;
	if (MovementComponent == nullptr) return;
	
	float LerpRatio = ClientTimeBetweenLastUpdate / ClientTimeSinceUpdate;
...
{
	FVector NewLocation = Spline.IterpolateLocation(LerpRatio);
...
{
	FVector NewDerivative = Spline.IterpolateDerivative(LerpRatio);
